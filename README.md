...
- `wechat`（默认）：问题转发微信，你的回复就是答案（仅微信会话；GUI 会话不受任何模式影响）
- `auto`：Agent 问了也白问——自动回「由你自行决定」，永不等待。注意：计划审批类
  问题在 auto 模式下相当于「不批准」（fail-closed），Agent 会自行决定后续动作
- `web`：保持原状，问题留在 Web GUI（微信侧无人感知）

被认领但投递失败的问题会直接让 ask 报错（不会回退到 GUI），避免两个人答同一道题。
